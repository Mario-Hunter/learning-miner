...
<div class="courseTicketprofile" > 
	 ...
	 <div style="width: 500px; height:200px;" class="courseName" ><a style="color: #7B671E" href="/courses/{{$course ->id}}">{{$course->name}}</a></div>
	 ...
	 <div class="subtitle course_style" style="width: 420px; height:40px;"><a id="livePrev" style="color: #D9BA4E" target = "_blank" href = {{$course->url}} > {{$course->url}}  </a><div class="box"><iframe src="https://en.wikipedia.org/" width = "500px" height = "500px"></iframe></div></div>
    ...
    <!-- @if($course->description != null)-->
	 ...
	 tags: 
		...
		<a style="color: #D9BA4E" href="/courses/tags/{{$tag->name}}"> {{$tag->name}}</a>
		@endforeach
	 </div>
	 @if(!Auth::guest())
		...
		@endif
		@endif
          <div style="width : 200px"class="rankEqual">Course Rank →&nbsp; {{ $course->rank }} </div>
</div>
